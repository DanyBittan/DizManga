...
<template>
    <div class="w-full max-h-[80vh] h-full flex justify-center items-centers">
        ...
        <div
            ...
            <div class="1/3">
                <img :src="props.comic.img" alt="comicImg" class="md:w-[40vw] w-full h-[40vh] md:h-full object-cover" />
            </div>
            <!-- Display of the main information about the comic -->
            <div class="flex flex-col text-gray-300 w-full h-full relative">
                <div class=" bg-neutral-900 rounded-tr-lg py-5 px-4 border-b border-neutral-700">
                    <h3 class="sm:text-3xl md:text-6xl w-[80%] text-2xl font-bold text-white line-clamp-2">{{
                        props.comic.title }}</h3>

                    <span class="text-xl text-neutral-400 font-bold w-1/2 md:w-auto mt-4 block">
                ...
                <span class="md:text-3xl text-xl text-blue-400 font-bold p-3 absolute top-2 right-2 drop-shadow-lg">
                    {{ props.comic.price }}€</span>
                <!-- Synopsis and remark of the title for easy understanding -->
                <div class="p-6 sm:p-6 flex flex-col gap-5 h-full bg-gradient-to-b from-black to-neutral-900">
                    <h3
                        class="sm:text-sm md:text-3xl w-[80%] text-xl font-bold text-gray-400 invisible md:visible line-clamp-1 overflow-hidden">
                        {{ props.comic.volume }}
                    </h3>
                    <div
                        ...
                        <div class="relative">
                            <div :class="synopsisExpanded ? 'max-h-[17rem] overflow-auto synopsis-scroll' : 'max-h-48 overflow-hidden synopsis-scroll'"
                                class="text-2xl leading-relaxed text-gray-200 transition-all">
                                <p class="whitespace-pre-line">{{ props.comic.sinopsis }}</p>
                            </div>

                            <!-- Fade overlay when collapsed -->
                            <div v-if="!synopsisExpanded"
                                class="pointer-events-none absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-neutral-900 via-transparent to-transparent">
                            </div>

                            <!-- Read more toggle -->
                            <button @click="synopsisExpanded = !synopsisExpanded"
    ...
    <div class="w-11/12 md:w-3/4 m-auto p-4 flex justify-between">
        <div class="flex">
            <button
                class="bg-neutral-900 border border-gray-600 text-gray-300 font-bold text-xl m-2 w-auto gap-3 px-3 py-2 rounded-lg flex hover:bg-neutral-800 hover:border-gray-500 transition"
                as="button">
                Buy
                <img src="../../assets/shoppingCart.svg" alt="shopping cart" class="w-7 h-7" />
            </button>
            <Link :href="route('saveWishlist', {
                id: props.comic.id,
                saved: wishlist,
                preserveScroll: true
            },)
                ">
                <button as="button"
                    class="bg-neutral-900 border border-gray-600 text-gray-300 font-bold text-xl m-2 w-auto px-3 py-2 rounded-lg gap-3 flex hover:bg-neutral-800 hover:border-gray-500 transition">
        ...
        <div>
            <button @click="showForm = !showForm" v-if="$page.props.auth.user?.admin"
            ...
            <Modal :show="showForm">
                <FormLayout @close-modal="showForm = !showForm" :comic="comic">
                    ...
                    <template v-slot:buttonType><img src="../../assets/edit.svg" alt="editBttn" class="w-5 h-5 mr-2" />
                        Update
                    </template>
                </FormLayout>
            </Modal>
